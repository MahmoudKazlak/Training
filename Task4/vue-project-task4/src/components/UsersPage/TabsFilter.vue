<template>
  <v-card
    ...
    class="card-height"
  >
    <v-row class="my-4 align-center rowRev">
      <!-- Search -->
      <v-col
        cols="12"
        md="5"
        class="mb-3 d-flex gap-3 align-items-center rowRev"
      >
        <v-text-field
          v-model="search"
          :label="$t('Search')"
        ...
        <!-- Export -->
        <IcoDropdown
          :items="[$t('Export as PDF'), $t('Export as Excel')]"
          :icons="[pdf, excel]"
        >
          <template #dropBtn>
            <v-btn
      ...
      <v-col
        cols="12"
        md="7"
        class="mb-3 d-flex gap-3 align-items-center justify-end control test"
      >
        <!-- Add user -->

        <UserDialog> <template #AddUser>{{ $t('Add User') }}</template> </UserDialog>
        <!-- Settings -->
